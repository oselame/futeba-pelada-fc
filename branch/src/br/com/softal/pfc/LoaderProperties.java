 ...
 * "catalina.properties" ( for backwad compatibility ) and will be searched in:
 * - TODO
 * 
 * Properties used: - TODO
 * 
 * loader.* and *.loader properties are used internally by the loader ( *.loader
 * is for backward compat with catalina ). All other properties in the config
 * file are set as System properties.
 * 
 * 
...
public class LoaderProperties {
	...
	/**
	 * Load properties. Will try: - "catalina.config" system property ( a URL )
	...
	private static void loadProperties() {
		...
		if (is == null) {
			try {
				is = LoaderProperties.class
						.getResourceAsStream("/org/apache/catalina/startup/catalina.properties");
				if (is != null && DEBUG) {
		...
		if (is == null) {
			try {
				is = LoaderProperties.class
						.getResourceAsStream("/org/apache/tomcat/util/loader/loader.properties");
				if (is != null && DEBUG) {
		...
		if (properties != null) {
			...
			while (enumeration.hasMoreElements()) {
				...
				if ("security.preload".equals(name))
					continue;
				if ("package.access".equals(name))
					continue;
				if ("package.definition".equals(name))
					continue;
	...
	static void setCatalinaBase() {
		...
		if (System.getProperty("catalina.home") != null)
			System.setProperty("catalina.base", System
	...
	static void setCatalinaHome() {
		...
		File bootstrapJar = new File(System.getProperty("user.dir"),
				"bootstrap.jar");
		File tloaderJar = new File(System.getProperty("user.dir"),
				"tomcat-loader.jar");
		if (bootstrapJar.exists() || tloaderJar.exists()) {
			try {
				System.setProperty("catalina.home", (new File(System
						.getProperty("user.dir"), "..")).getCanonicalPath());
			} catch (Exception e) {
				// Ignore
